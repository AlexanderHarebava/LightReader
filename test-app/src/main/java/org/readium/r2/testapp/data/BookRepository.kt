...
class BookRepository(
    ...
    // BookRepository.kt
    suspend fun insertBookIfNotExists(
        url: Url,
        mediaType: MediaType,
        publication: Publication,
        cover: File,
    ): Long {
        // Проверяем, существует ли уже книга с таким href
        if (booksDao.bookExists(url.toString()) > 0) {
            return -1 // или выбросить исключение, или вернуть существующий id
        }
    ...
    suspend fun insertReadingSession(session: org.readium.r2.testapp.data.model.ReadingSession) {
        booksDao.insertReadingSession(session)
    }

    fun getAllReadingSessions(): Flow<List<org.readium.r2.testapp.data.model.ReadingSession>> =
        booksDao.getAllReadingSessions()

    fun getReadingSessionsForBook(bookId: Long): Flow<List<org.readium.r2.testapp.data.model.ReadingSession>> =
        booksDao.getReadingSessionsForBook(bookId)

    fun getTotalReadingTimeSince(since: Long): Flow<Long?> =
        booksDao.getTotalReadingTimeSince(since)
    ...
    suspend fun getAllHrefs(): Set<String> {
        return booksDao.getAllHrefs().toSet()
    }
