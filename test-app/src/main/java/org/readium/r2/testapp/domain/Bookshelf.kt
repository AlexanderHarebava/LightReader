...
class Bookshelf(
    ...
    suspend fun addLocalFilesConcurrently(
        urls: List<AbsoluteUrl>,
        concurrency: Int = 4,
    ): Int = coroutineScope {
        ...
        urls.map { url ->
            launch(Dispatchers.Default) {
                semaphore.withPermit {
                    addLocalFile(url).onSuccess { addedCount.incrementAndGet() }
                }
            }
        }.joinAll()
        addedCount.get()
    ...
    private suspend fun addBook(
        ...
        publicationOpener.open(
            ...
            val id = bookRepository.insertBookIfNotExists(
                url,
            ...
            if (id == -1L) {
                coverFile.delete()
                return Try.failure(
                    ImportError.Database(DebugError("Book already exists or could not be inserted."))
                )
            }
        }
            .onFailure {
    ...
    // Bookshelf.kt
    // Bookshelf.kt
    suspend fun addLocalFile(url: AbsoluteUrl): Try<Unit, ImportError> {
        return addBook(url)
    }
