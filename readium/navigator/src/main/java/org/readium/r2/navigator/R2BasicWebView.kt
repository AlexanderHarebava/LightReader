...
internal open class R2BasicWebView(context: Context, attrs: AttributeSet) : WebView(context, attrs) {
    ...
    init {
        // Отключаем плавную прокрутку, чтобы избежать эффекта "пружины" при баге
        overScrollMode = OVER_SCROLL_NEVER
// Для старых версий Android иногда помогает это:
    ...
    val progression: Double get() {
        return if (scrollMode) {
            ...
            if (isRtl) {
                ...
                // LTR: делим на maxScroll, чтобы последняя страница давала 1.0
                val maxScroll = contentWidth - pageWidth
                if (maxScroll > 0) {
                    (x / maxScroll).coerceIn(0.0, 1.0)
                } else {
                    0.0
                }
            }
    ...
    override fun onOverScrolled(scrollX: Int, scrollY: Int, clampedX: Boolean, clampedY: Boolean) {
        if (isSelecting) {
            // Игнорируем полностью, не вызываем super
            return
        }
    ...
    open fun onSelectionEnd() {
        if (lastSelectionState) {
            ...
            isSelecting = false
            Timber.d("Selection ENDED")
        }
    }

    @android.webkit.JavascriptInterface
    fun isPageAnimating(): Boolean {
        // mScrollState == 2 (SCROLL_STATE_SETTLING) — это и есть анимация
        return (this as? R2WebView)?.isSettling == true
    }
    @android.webkit.JavascriptInterface
    fun isSelecting(): Boolean = isSelecting  // уже есть поле
    /** Produced by gestures.js */
    ...
    override fun scrollTo(x: Int, y: Int) {
        if (isSelecting && (x != scrollX || y != scrollY)) return
        super.scrollTo(x, y)
    }

    override fun scrollBy(x: Int, y: Int) {
        if (isSelecting && (x != 0 || y != 0)) return
        super.scrollBy(x, y)
    }
