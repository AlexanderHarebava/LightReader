...
public data class HtmlDecorationTemplate(
    ...
    public companion object {
        ...
        private fun createTemplate(
            ...
            val alphaCss = numberFormat.format(alpha)
            val activeAlphaCss = numberFormat.format((alpha * 2).coerceAtMost(0.8))
            // На чёрном фоне полупрозрачная подсветка тусклая — в ночной теме усиливаем.
            val nightAlphaCss = numberFormat.format((alpha * 1.5).coerceAtMost(0.75))
            val nightActiveAlphaCss = numberFormat.format((alpha * 2.5).coerceAtMost(0.9))
            ...
            return HtmlDecorationTemplate(
                ...
                element = { decoration ->
                    ...
                    val rgb = "${Color.red(tint)}, ${Color.green(tint)}, ${Color.blue(tint)}"
                    val css = buildString {
                        append("--r2-tint-rgb: $rgb;")
                        if (asHighlight) {

                            val alphaVar = if (isActive) "--r2-active-alpha" else "--r2-alpha"
                            val alphaFallback = if (isActive) activeAlphaCss else alphaCss
                            append("background-color: rgba(var(--r2-tint-rgb), var($alphaVar, $alphaFallback)) !important;")

                            append("border: 0 !important;")
                        } else {

                            if (isActive) {
                                append("background-color: rgba(var(--r2-tint-rgb), var(--r2-active-alpha, $activeAlphaCss)) !important;")
                            }

                            append("border-color: rgb(var(--r2-tint-rgb)) !important;")
                        }
                        append("display: block !important;")
                        append("visibility: visible !important;")
                        append("opacity: 1 !important;")
                        append("pointer-events: none;")
                        append("z-index: 9999 !important;")
                        append("box-sizing: border-box;")
                        append("border-radius: ${cornerRadius}px;")
                        append("width: 100% !important;")
                        append("height: 100% !important;")
                        append("contain: layout style !important;")
                        append("isolation: isolate;")
                    }
                    """<div class="$className" style="$css"/>"""
                },
                stylesheet = buildString {
                    append(
                        """
                .$className {
                    margin: 0 !important;
                ...
                :root[style*="readium-night-on"] .$className {
                    mix-blend-mode: screen;
                    --r2-alpha: $nightAlphaCss;
                    --r2-active-alpha: $nightActiveAlphaCss;
                }
                """.trimIndent()
                    )
                    ...
                    if (!asHighlight) {
                        append(
                            """
                    .$className {
                        border: 0 solid var(--underline-color, transparent);
                    }
