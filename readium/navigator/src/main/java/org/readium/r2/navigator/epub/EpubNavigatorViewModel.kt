...
internal class EpubNavigatorViewModel(
    ...
    fun onResourceLoaded(webView: R2BasicWebView, link: Link): List<RunScriptCommand> =
        buildList {
            ...
            // Применяем декорации для текущего ресурса
    ...
    fun clearSelection(): RunScriptCommand =
        RunScriptCommand(
            """
        (function() {
            if (window.customSelector && typeof window.customSelector.clearSelection === 'function') {
                window.customSelector.clearSelection();
            } else {
