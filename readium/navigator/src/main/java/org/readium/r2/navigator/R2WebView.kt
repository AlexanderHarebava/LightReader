...
internal class R2WebView(context: Context, attrs: AttributeSet) : R2BasicWebView(context, attrs) {
    ...
    internal val isSettling: Boolean get() = mScrollState == SCROLL_STATE_SETTLING
    ...
    override fun onScrollChanged(l: Int, t: Int, oldl: Int, oldt: Int) {
        ...
        // КРИТИЧНО: Если нативный авто-скролл сдвинул страницу (например, во время выделения),
        // мы должны обновить mCurItem, иначе свайпы перестанут работать или будут перелистывать
        // не ту страницу.
        if (isSelecting) {
            val clientWidth = getClientWidth()
            if (clientWidth != null && clientWidth > 0) {
                val newItem = (l.toDouble() / clientWidth.toDouble()).roundToInt()
                if (newItem != mCurItem) {
                    mCurItem = newItem
    ...
    override fun computeScroll() {
        ...
        // Оригинальная логика только если НЕ идет выделение
        if (scrollMode) {
    ...
    override fun onTouchEvent(event: MotionEvent?): Boolean {
        ...
        // ----- НОВОЕ: во время выделения запрещаем родителю перехватывать тач -----
        // Это критично: иначе ViewPager на границе gutter'а может "украсть" тач
        // и начать перелистывать страницу прямо под пальцем пользователя,
        // тащащим маркер выделения.
        if (isSelecting) {
            parent?.requestDisallowInterceptTouchEvent(true)
        }

        // Если идёт выделение — отдаём событие WebView, но НЕ запускаем свою
        // логику свайпа/перелиста. Маркеры выделения должны двигаться плавно.
        if (isSelecting) {
            // Сбрасываем любой "накопленный" drag-стейт, чтобы при отпускании
            // пальца мы не попытались случайно перелистнуть страницу.
            mIsBeingDragged = false
            mVelocityTracker?.clear()
        ...
        // -----------------------------------------------------------------------
        // Ниже — ОРИГИНАЛЬНАЯ логика без изменений.
        // -----------------------------------------------------------------------

        // 1. Инициализация трекера скорости
        if (mVelocityTracker == null) {
            mVelocityTracker = VelocityTracker.obtain()
        }
        event.let { mVelocityTracker?.addMovement(it) }
        ...
        when (action) {
            ...
            MotionEvent.ACTION_MOVE -> {
                ...
                if (!mIsBeingDragged) {
                    ...
                    if (pointerIndex >= 0) {
                        val x = event.safeGetX(pointerIndex)
            ...
            MotionEvent.ACTION_UP -> {
                if (mIsBeingDragged) {
                    ...
                    val x = event.safeGetX(activePointerIndex)
                    val y = event.safeGetY(activePointerIndex)

                    if (scrollMode) {
                        val totalDelta = (y - mInitialMotionY).toInt()
                        if (abs(totalDelta) < 200) {
                            if (mInitialOverscroll == OverscrollMode.BOTH) {
                                if (mInitialMotionX < x) scrollLeft(animated = true)
                                else if (mInitialMotionX > x) scrollRight(animated = true)
            ...
            MotionEvent.ACTION_POINTER_UP -> {
                ...
                mLastMotionX = event.safeGetX(event.findPointerIndex(mActivePointerId))
            }
        }
