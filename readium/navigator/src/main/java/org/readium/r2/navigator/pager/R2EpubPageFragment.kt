...
//Или удалить webView.setLayerType
        ...
        webView.webViewClient = object : WebViewClientCompat() {
            ...
            override fun onPageFinished(view: WebView?, url: String?) {
                ...
                val refreshDecorationsScript = """
    (function() {
        if (window.readium && typeof readium.getDecorations === 'function') {
            setTimeout(function() {
                var groups = ['highlights', 'search', 'tts'];
                groups.forEach(function(groupName) {
                    try {
                        var group = readium.getDecorations(groupName);
                        if (group && group._decorations && group._decorations.length > 0) {
                            // Триггерим перерисовку без удаления
                            var decorations = Array.from(group._decorations.values());
                            group.clear();
                        ...
                        console.log('Error refreshing decorations:', e);
                    }
                });
            }, 200);
        }
    ...
    """
                webView.evaluateJavascript(refreshDecorationsScript, null)
                onPageFinished()
    ...
    private suspend fun loadLocator(
        ...
        if (webView.scrollMode) {
            ...
            // progression.
            var item = if (readingProgression == ReadingProgression.RTL) {
                // RTL: сохраняем оригинальную логику
                (progression * webView.numPages).roundToInt()
            } else if (webView.numPages > 1) {
                // LTR: progression теперь [0..1] → item [0..numPages-1]
                (progression * (webView.numPages - 1)).roundToInt()
            } else {
                0
            }
